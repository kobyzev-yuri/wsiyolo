"""
Основной модуль для WSI YOLO pipeline.
Объединяет все компоненты для обработки WSI с YOLO моделями.
"""
...
class WSIYOLOPipeline:
    """Основной pipeline для WSI YOLO анализа"""
    ...
    def __init__(self, models_config: List[Dict[str, Any]], 
        ...
        self.wsi_pipeline = WSIPipeline(tile_size, overlap_ratio)
    ...
    def process_wsi(self, wsi_path: str, output_path: str = None) -> List[Prediction]:
        """
        ...
        # 4. Объединяем перекрывающиеся предсказания
        print("🔗 Объединение перекрывающихся предсказаний...")
        merged_predictions = self.polygon_merger.merge_predictions(all_predictions)
