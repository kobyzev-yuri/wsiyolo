#!/usr/bin/env python3
"""
Извлечение реальных патчей с предсказаниями из WSI
"""
...
def extract_patches_with_predictions(json_path, wsi_path, output_dir="real_patches", max_patches=None):
    ...
    for i, ((patch_x, patch_y), patch_predictions) in enumerate(selected_patches):
        ...
        # Сохраняем с новым именем: wsi_name.patch_i.patch_j_real.png
        output_file = output_path / f"{wsi_base_name}.patch_{patch_i}.{patch_j}_real.png"
...
if __name__ == "__main__":
    ...
    print("🎨 Извлечение реальных патчей с предсказаниями")
    print("=" * 50)
    
    # Извлекаем реальные патчи (ВСЕ патчи с предсказаниями)
